...
from global_functions import get_global_data,update_global_data,get_wordlist,is_relevant_sentence,add_reference,remove_file
...
def is_text(text):
	if (len(text) == 0):
		return False
	if (len(text.split(" ")) <= 2):	#Atleast 3 words
		return False
...
def index_relevant_words(sentences,file_path):
	...
	for sentence_index,sentence in enumerate(sentences):
		for word in words:
			if (word in sentence and is_relevant_sentence(sentence,word)):
...
if __name__ == "__main__":
	...
	for link in links:
		...
		file_path = os.path.join(global_data['articles']['path'],file_name)
